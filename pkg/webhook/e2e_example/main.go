...
func main(cmd *cobra.Command, args []string) {
	...
	server := &http.Server{
		...
		// TLSConfig 可选择提供 TLS 配置以供 ServeTLS 和 ListenAndServeTLS 使用。
		//请注意，该值是由 ServeTLS 和 ListenAndServeTLS 克隆的，
		//因此无法使用 tls.Config.SetSessionTicketKeys 等方法修改配置。
		//要使用 SetSessionTicketKeys，
		//请使用带有 TLS 侦听器的 Server.Serve。
