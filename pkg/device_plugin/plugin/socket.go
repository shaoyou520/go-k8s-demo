...
func (scl SocketCANLister) GetResourceNamespace() string {
	return common.ResourceNamespace
}

func (scl SocketCANLister) Discover(pluginListCh chan manage.PluginNameList) {
	all_devices := append([]string{"vcan"}, scl.Real_devices...)
	var plugins = manage.PluginNameList(all_devices)

	pluginListCh <- plugins
}

func (scl SocketCANLister) NewPlugin(kind string) manage.PluginInterface {
